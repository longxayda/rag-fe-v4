...
import { Search, Grid, List, MapPin, Landmark, X, Filter, Sparkles, LayoutGrid } from 'lucide-react';
...
export default function HeritageListPage() {
  ...
  const [viewMode, setViewMode] = useState('bento'); // Default to bento view
  ...
  const categorizeHeritageType = (notes) => {
    ...
    // Map notes content to simplified categories
    ...
    return 'heritage'; // Default category
  };
  ...
  const filteredData = useMemo(() => {
    return allData.filter(item => {
      ...
      if (filters.type) {
        // For heritage items, check the categorized type based on notes
        if (item.dataType === 'heritage') {
          ...
          // For people and festivals, match exact data type
          matchesType = false; // Type filter only applies to heritage items for now
        }
      }
